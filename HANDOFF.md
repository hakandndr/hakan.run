...
| Exact next action | Export a fresh `hakanrun_panel_log.txt` from production immediately before importing, run `node tools/legacy-analytics/plan-legacy-import.js <path>` against it, and review the plan. The old snapshot is development input only: production keeps appending, so its counts describe a file that no longer exists |
