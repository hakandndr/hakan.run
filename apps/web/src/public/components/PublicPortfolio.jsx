...
const PublicPortfolio = ({ portfolio }) => (
  <section
    id="portfolio"
    data-public-section="portfolio"
    className="relative overflow-hidden py-24 border-t border-white/[0.06]"
    style={{ backgroundColor: '#0B0B0C' }}
  >
    <div className="container mx-auto px-6">
      <div className="mb-14">
        <span className="font-mono text-xs text-[#57B8FF]/60 uppercase tracking-widest">{portfolio.badge}</span>
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight uppercase font-mono tracking-tight mt-2">
          {portfolio.heading} <span className="text-[#57B8FF]">{portfolio.headingAccent}</span>
        </h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {portfolio.cards.map((project) => (
          <a
            key={project.id}
            data-portfolio-card
