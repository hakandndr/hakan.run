          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] xl:grid-cols-[1fr_360px] gap-6 lg:gap-8 items-center">
            ...
            >
              ...
              <h1 className="text-5xl md:text-6xl lg:text-7xl xl:text-[88px] font-bold mb-8 leading-[1.05] text-white uppercase font-mono tracking-tight">
                {hero.headingLine1}
                <span className="block text-[#57B8FF]">{hero.headingLine2}</span>
              </h1>

              <div className="mb-10 max-w-lg">
                <p className="text-[17px] text-[#A1A1AA] leading-[1.75]">{hero.paragraph}</p>
              </div>

              <div className="flex flex-wrap gap-3 mb-8">
                <Button
                  onClick={() => navigate(hero.primaryButtonHref)}
                  size="lg"
                ...
                <Button
                  onClick={() => navigate(hero.secondaryButtonHref)}
                  size="lg"
                  variant="outline"
            ...
            >
              <div className="relative w-full max-w-[300px] lg:max-w-[320px] xl:max-w-[340px]">
                <div className="absolute top-6 -left-5 z-20 bg-[#181818] border border-white/[0.12] rounded-xl px-3 py-2.5 shadow-xl">
                  ...
                  <p className="text-[11px] text-[#9CA3AF] tracking-wide whitespace-nowrap mt-0.5">{hero.profile.topLabel}</p>
                </div>
                ...
                <div className="absolute bottom-6 -right-3 z-20 bg-[#181818] border border-white/[0.12] rounded-xl px-3 py-2.5 shadow-xl text-right">
                  <p className="text-[11px] text-[#9CA3AF] tracking-wide whitespace-nowrap">{hero.profile.bottomLabel}</p>
                  <p className="font-mono text-sm font-bold text-white whitespace-nowrap mt-0.5">{hero.profile.bottomValue}</p>
                </div>
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 px-5 py-4 bg-gradient-to-t from-black/75 via-black/30 to-transparent">
                    <p className="font-mono text-xs font-bold text-white/80 uppercase tracking-widest">{hero.profile.name}</p>
                    <p className="font-mono text-[10px] text-white/40 mt-0.5">
                      {hero.profile.role} · {hero.profile.location}
                    </p>
                  </div>
                </div>
              </div>
